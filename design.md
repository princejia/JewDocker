...
- **报价详情页**：可修改客户名称；选取在库产品（可按编号/名称搜索，接口单页上限 100，逐页拉全）后自动带入产品金额，输入折扣后实时算出报价金额，点【报价】生成一条报价记录，可继续选其他产品继续报
- **折扣为乘数**：`1` = 原价，`0.85` = 85 折；报价金额 = 产品金额 × 折扣（四舍五入到分），产品金额可手动覆盖
- **转为销售**：每条报价记录旁有【转为销售】按钮，点击后按报价金额写入 `product_sales`（备注「由报价转入」），同步把产品置为【已售】，并回写 `sale_id`；已转的记录不可重复转、不可修改或删除
