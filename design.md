...
- 版式针对 **Godex G530 的 30×25mm 对折标**，折痕在 30mm 正中，内容不跨越折痕：
  - **左半面**：二维码 12×12mm（编码指向公开展示页 `/v/<p|s>/<id>`，`p` = 产品，`s` = 裸石），下方居中打印**售价**
  - **右半面**：产品名称（最多 2 行自动折行），下方依次为尺寸、产地、重量（含单位）、镶嵌配石（最多 3 行自动折行）、工费、附加费，**只打印值本身，不带字段名**
  - **字段为空或为 0 时整行省略**，右半面写满即止
