...
> **利润列口径**：表格视图、只读详情页与 Excel 导出的利润均为 `出售价 − purchaseCostOf(p)`，**仅已售产品显示**，在库/借售显示 `-`。数据库生成列 `profit`（`price − purchase_price`）对黄金无意义（`purchase_price` 存的是每克金价），已不再在界面使用。Excel 另增「进货成本(¥)」列。
...
- **点击物品名称查看详情**：销售流水中点击产品/裸石名称可打开对应的**只读详情页**（`/products/[id]/view`、`/loose-stones/[id]/view`）
- 付款状态跟踪：支持记录部分付款
- **退货管理**：在销售页登记退货（关联某笔销售，自动带出产品/客户/退款金额），可编辑、可删除；登记后产品自动恢复为【在库】
- 时间段汇总：总销售额（已扣除退款）、总利润、平均客单价
- **按月统计**：销售页顶部卡片下方按月汇总——成交笔数、销售额、退款、净销售额、平均客单价；销售按成交日期归属，退款按退货日期归属，月份倒序；默认只展示最近 12 个月，可展开全部

### 4.4 报价管理模块 `/quotes`

- 列表页按报价时间倒序展示全部报价单：客户、报价件数、合计金额、已转销售件数、报价时间、备注；点击客户名进入详情页修改
- **新建报价**：弹窗输入客户名称（带已有客户名的 datalist 联想）与备注，创建后直接跳转详情页继续报价
...
- **报价记录悬停预览**：明细表里的产品**不可点击跳转**，鼠标移上去弹出跟随光标的详情卡片（复用 `ProductHoverPreview`）；添加报价的产品下拉里，鼠标移到选项上同样弹出该卡片
- **非黄金计价**：产品金额（自动带入售价，可改）× 折扣 = 报价金额
- **黄金计价**（宝石分类为「黄金」时自动切换）：工费销售价格与附加费自动带入产品档案，各配一个折扣输入；
  - 工费小计 = 工费销售价格(g/元) × 克重 × 工费销售折扣
  - 附加费小计 = 附加费 × 附加费折扣
  - 金价小计 = 当日金价(g/元) × 克重（当日金价必填，每次报价现填）
  - 销售价格 = 工费小计 + 附加费小计 + 金价小计
- **算价在服务端重算**：接口只接收原始输入，各项小计与最终价由 `computeQuotePricing()` 统一算出并快照进明细，前端同函数做实时预览
...
- **查看已转销售的报价详情**：已转销售的记录，点【报价编号】或操作列的【查看详情】弹出只读弹窗，展示写入当时的计价快照——非黄金为产品金额与折扣，黄金为工费/附加费/金价三段的单价、折扣与小计，另含克重、报价时间与最终销售价格
...
- **点击物品名称查看详情**：借调列表中点击产品/裸石名称可打开对应的**只读详情页**（`/products/[id]/view`、`/loose-stones/[id]/view`）
...
| 按月汇总 | 按月份汇总成交笔数、销售额、退款、净销售额、成本、利润、利润率，月份倒序（销售按成交日期归属，退款按退货日期归属）；默认只展示最近 12 个月，可展开全部 |
| 未结款汇总 | 列出借售中未完全收款记录，支持催款标记 |
| 已售产品利润明细 | 全部成交流水的逐笔明细（编号、名称、出售时间、出售价、进货成本、工费/附加费/金价三段利润、利润），支持按编号/名称搜索，实时汇总件数、出售价合计与利润合计；可**导出 Excel**（导当前筛选结果，末尾带合计行） |
| 在库总成本 | 当前在库物品的进货成本合计 |
...
>
> **退款口径（仪表盘 / 销售页 / 报表页一致）**：退款按退货日期归属，同时直接抵减当期销售额与利润（报表页的「近 30 天销售趋势」与「按月汇总」均适用）；「已售产品利润明细」是逐笔流水，不做退款净额化。
...
**黄金利润的三段拆解**（`goldProfitParts()`，仅黄金分类有，三项之和恒等于该行利润）：

| 拆解项 | 公式 |
|--------|------|
| 工费利润 | 销售工费小计 − 工费成本(g/元)×克重 |
| 附加费利润 | 销售附加费小计 − 附加费×买入折扣 |
| 金价利润 | (成交价 − 销售工费小计 − 销售附加费小计) − 进货价(g/元)×克重 |

> 销售侧的工费/附加费小计**优先取报价快照**（`quote_items.labor_subtotal` / `surcharge_subtotal`，已含销售折扣）；直接登记、未走报价的销售按产品档案的工费销售价格×克重与附加费估算。金价部分取成交价扣除前两项的余额，保证拆解结果与**实际成交价**对齐（而非报价价）。
...
- 版式针对 **Godex G530 的对折标**：走纸时 25mm 边先出，因此 PDF 页面为 **25mm(宽) × 30mm(高)** 竖向，折痕是 y=15mm 的横线，把标签分成上下两个 25×15mm 的可见面，内容不跨越折痕；四周留 1.2mm 不可打印安全边：
  - **上面**：二维码 10×10mm 贴外沿放置（编码指向公开展示页 `/v/<p|s>/<id>`，`p` = 产品，`s` = 裸石），右侧打印**售价**；该面内容整体旋转 180°，对折后与下面同向
  - **下面**：产品名称（最多 2 行自动折行），下方依次为尺寸、产地、克重（前缀 `克重：`，含单位）、镶嵌配石（最多 2 行自动折行）、工费与附加费合并一行（`工费：xx  F：xx`）
...
- 识别成功后进入公开展示页 `/v/<p|s>/<id>`，根据登录状态自动分流：
  ...
  - **未登录**（手机相机直扫）：进入清新美观的展示页，支持多图展示与缩略图切换、点击图片全屏放大；字段与实体标签同口径——名称、编号、**售价**、重量、尺寸、产地、宝石/功能分类、镶嵌配石、工费(g/元)、附加费(总额)，空字段自动省略；**进货价、工费成本、利润等成本数据不在查询列白名单内**，页面源码里也拿不到
...
- 无需登录即可浏览完整目录，产品与裸石按创建时间倒序合并展示，**含已售商品**
- 支持「全部 / 产品 / 裸石」切换与按编号、名称检索；点击卡片打开全屏预览，支持缩略图切换、左右方向键与 Esc 关闭
- 预览弹窗底部信息区与扫码详情页 `/v/<p|s>/<id>`、实体标签同口径：编号、名称、**售价**、重量、尺寸、产地、分类、镶嵌配石、工费、附加费，空字段自动省略；内容过高时该区域独立滚动，不挤压图片
...
| /loose-stones/[id]/view | 裸石详情（只读） | 只读查看裸石完整信息，右上角提供【编辑】入口；销售/借调页点击裸石名进入 |
